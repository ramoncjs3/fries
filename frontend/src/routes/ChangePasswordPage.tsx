...
// 只做「两次输入一致」这种纯前端能判的校验。
// **密码强度不在这里硬编码** —— 那是 settings 表里的策略，后端说了算，
// 前端复制一份就一定会和后端不一致（DECISIONS.md §5）。
const schema = z
  .object({
...
export default function ChangePasswordPage() {
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => changeOwnPassword(values.old_password, values.new_password),
    onSuccess: async () => {
      toast.success('密码已修改')
